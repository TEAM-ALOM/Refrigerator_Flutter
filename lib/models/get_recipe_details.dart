...
class Recipe {
  ...
  Map<String, bool> ingredientMap; // 필요한 재료 배열 (재료이름, 보유여부)
...
// Recipe 멤버값 채우는 메서드
  void setRecipe(String id, String title, String category, String thumbnail,
    ...
    ingredientMap = Map.fromIterable(
      ingredientList,
      key: (item) => item['name'] as String, // name 값이 key가 됨
      value: (item) => item['isContained'] as bool, // isContained 값이 value가 됨
    );
  }
...
Future<void> getRecipeDetails(String foodName) async {
  final Uri url =
      Uri.parse('http://43.201.84.66:8080/api/recipe/search?keyword=$foodName');
